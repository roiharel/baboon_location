...
#   library(shiny)
#   
#   # Define choices for the new column
#   choices <- c("Rest", "Monitor", "High")
#   
#   # Add an editable column with a dropdown menu
#   last_rows_per_tag_html$'collar_state' <- mapply(function(status) {
#     sprintf(
...
#        <option value="%s" %s>%s</option>
#      </select>',
#       choices[1], ifelse(status == choices[1], "selected", ""), choices[1],
#       choices[2], ifelse(status == choices[2], "selected", ""), choices[2],
#       choices[3], ifelse(status == choices[3], "selected", ""), choices[3]
#     )
#   }, last_rows_per_tag_html$status)
#   
#   # Create the interactive datatable
#   interactive_table <- datatable(
#     last_rows_per_tag_html,
#     escape = FALSE,  # Allow HTML rendering for the dropdown
#     options = list(
#       paging = TRUE,
#       searching = TRUE,
#       ordering = TRUE,
